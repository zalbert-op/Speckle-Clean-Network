...
def get_root_logger(logger_name='basicsr',
    ...
    color_formatter = colorlog.ColoredFormatter(
        fmt='%(asctime_log_color)s%(asctime)s%(reset)s %(log_color)s%(levelname)-4s%(reset)s %(message_log_color)s%(message)s',
        datefmt='%Y-%m-%d %H:%M:%S',
        log_colors={
            'DEBUG': 'cyan',
            'INFO': 'blue',
    ...
    if log_file and not any(isinstance(h, logging.FileHandler) for h in logger.handlers):
        file_handler = logging.FileHandler(log_file, 'a')  # Use 'a' for append mode
        file_handler.setFormatter(plain_formatter)  # Use plain formatter for file output
