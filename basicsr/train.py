...
def init_loggers(opt):
    ...
    logger = get_root_logger(
        logger_name='basicsr1', log_level=logging.INFO, log_file=log_file)
...
def main():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0 or current_iter == 1000):
                ...
                logger.info('Saving models with latest validation metrics.')
                model.save(epoch, current_iter,
                           psnr=model.psnr_value,
                           ssim=model.ssim_value)
    ...
    if opt.get('val') is not None:
        gray2bgr = opt['val'].get('gray2bgr', False)
        use_image = opt['val'].get('use_image', True)
        metric = model.validation(val_loader, current_iter, tb_logger,
                         opt['val']['save_img'], gray2bgr, use_image)
    if tb_logger:
        tb_logger.close()
